...
ACTUAL_PATH = os.getcwd()
HP_WORKING_DIR = ACTUAL_PATH + "/HoneyPot_Files"
...
# FUNCION PARA PILLAR EL PUERTO Y LA IP EN LA QUE SE LANZA EL HP
def get_host_and_port_from_config(config_file):
    try:
        ...
        print("Error al decodificar el archivo JSON de configuración.")
        return "", ""
    
def get_user_pass_directory_from_config(config_file):
    try:
        with open(config_file, "r") as file:
            config_data = json.load(file)
            ssh_config = config_data.get("ssh", {})
            userdb_directory = ssh_config.get("userdb", "")
            return userdb_directory
    except FileNotFoundError:
        print("Archivo de configuración no encontrado.")
        return ""
    except json.JSONDecodeError:
...
USERDB_FILE = get_user_pass_directory_from_config(CONFIG_FILE)
USERDB_FILE = ACTUAL_PATH + USERDB_FILE
...
def handle_connection(client, addr):
    ...
    try:
        ...
        try:
            ...
            log_file = os.path.join("LOGS", "dir.log")
            ...
            while run:
                ...
                if command == "exit":
                    ...
                    # EJECUTA EL COMANDO
                    args = ["python3", "../main.py", command, client_ip]
...
if __name__ == "__main__":
    ...
    host, port = get_host_and_port_from_config(CONFIG_FILE)
    print('Listening for connection ...')
